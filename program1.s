...
main:
  ...
  lw $t5, 16($zero) //carga un 16 en reg13
  lw $t6, 20($zero) //carga un 32 en reg14
  ...
  sw $t5, 40($zero) //guarda en pos40 de datamem el dato en reg13 (un 16)
  sw $t6, 44($zero) //guarda en pos44 de datamem el dato en reg14 (un 32)
  lw $t1, 24($zero) //guarda en pos24 de datamem el dato en reg9 (un 1) 
  lw $t2, 28($zero) //guarda en pos28 de datamem el dato en reg10 (un 2)
  lw $t3, 32($zero) //guarda en pos32 de datamem el dato en reg11 (un 4)
lab1:
  ...
  slt $s7, $t1, $t2 //guarda en reg23 un
  slt $t8, $s0, $t2
  lui $t1, 1
  lui $t2, 2
  addi $t3, $t3, 6 
